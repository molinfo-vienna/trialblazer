...
def target_features_preprocess(chembl_data, preprocessed_chembl_data, outputFolder, remove_multi_components=False):
    if remove_multi_components:
        removed_multicom_chembl = preprocessed_chembl_data[~preprocessed_chembl_data.id.str.contains("x")]
        print(f"Number of multi-component compounds removed: {len(preprocessed_chembl_data) - len(removed_multicom_chembl)}")
    else:
    ...
    merged_data = removed_multicom_chembl.merge(chembl_data[['id', 'target_id']], how='left', on='id')
    merged_data['MolWithoutStereo'] = merged_data['preprocessedSmiles'].apply(Chem.MolFromSmiles)
    merged_data['SmilesWithoutStereo'] = merged_data['MolWithoutStereo'].apply(lambda x: Chem.MolToSmiles(x, isomericSmiles=False))
    target_label_smpl = merged_data[['target_id', 'SmilesWithoutStereo']].dropna(subset=['target_id'])
    target_id_list, preprocessed_target_unique_smiles, fpe = generate_h5file(target_label_smpl, outputFolder)
    return target_id_list, preprocessed_target_unique_smiles, fpe

def tanimoto_similarity_calculation(fpe, query_dataset, split_data=False, num_splits=None):
    results_append = []
    my_smi_append = []
    if split_data:
        if not num_splits:
            raise ValueError("num_splits must be provided when split_data is True.")
...
def generate_h5file(preprocessed_target, outputFolder):
    ...
    preprocessed_target_unique_smiles = preprocessed_target.groupby('SmilesWithoutStereo')['target_id'].apply(list).reset_index()
    list_smi = preprocessed_target_unique_smiles['SmilesWithoutStereo'].tolist()
    print(f"numbers of unique SMILES:{len(list_smi)}")
    create_db_file(list_smi, outputFolder, 'Morgan', {'radius': 2, 'nBits': 2048})
