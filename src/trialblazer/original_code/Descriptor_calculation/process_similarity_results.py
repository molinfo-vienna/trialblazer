...
def separate_similarity_results(results, target_id_list, preprocessed_target_unique_smiles, output_path_temp_save, start_index=0):
    default_value = -1
    for i, array in tqdm(generator_separate_results(results, start_index=start_index)):
        ...
        for value in array:
            p = preprocessed_target_unique_smiles.iloc[value[0]-1]['target_id']
            if any(item in target_id_list for item in p):
                for target in p:
                    if value[1] > sim_dict[target]:
                        sim_dict[target]=value[1]
        dict_save.append(sim_dict)
        smi_save.append(smi)
        temp_save['smi'] = smi_save
        temp_save['dict'] = dict_save
        temp_save.to_csv(Path(output_path_temp_save +'/'+'temp_save_' + str(i)+'.csv'), sep='|')
    print("the separation of simialrity results is finished!")

def organize_similarity_results(target_id_list, output_path_temp_save, training_data=False):
...
def binarize_similarity_value(dataframe, target_list, threshold):
    dataframe[target_list] = dataframe[target_list].apply(lambda row: row.map(lambda x: threshold_binarize(x, threshold)))
    return target_list, dataframe

def remove_tested_inactive_targets(binarized_inactive_target, binarized_query_targets):
    filtered_rows = []
    for i, targets in binarized_inactive_target.iterrows():
        ...
        active_target_indices = set(active_targets[active_targets == '1'].index)
...
def remove_invariant_target(sanity_checked_dataframe, target_list):
    ...
    print("the number of removed targets: {}".format(len(target_remove)))
    print("the number of remaining targets: {}".format(len(target_list)))
    binarized_target_remain = sanity_checked_dataframe[target_binarize_list]
    binarized_target_remain['SmilesForDropDu'] = sanity_checked_dataframe['SmilesForDropDu']
...
def generator_separate_results(results, start_index=0):
    for i, array in enumerate(results.similarity_results[start_index:], start=start_index):
        yield i, array
