...
def plot_score_distribution_with_significance(data, p_values):
    ...
    ax = sns.violinplot(x="Category", y="Value", data=data, palette=custom_palette, alphas=0.8,inner="quartile")
...
def plot_correlation(predict_result_toxic, predict_result_benign):
    ...
    plt.scatter(predict_result_toxic['mw'], 
                predict_result_toxic['PrOCTOR_score'], 
                color='#a17db4', alpha=0.9,label='Predicted toxic drugs')
    
    plt.scatter(predict_result_benign['mw'], 
                predict_result_benign['PrOCTOR_score'], 
                color="#8ea5c8", alpha=0.9, label='Predicted benign drugs')
    ...
    plt.text(633, 2.3, f"Prediction threshold = {prediction_threshold:.2f}", fontsize=10, color='black')
    plt.text(140, 27, f"r (Pearson) = {correlation:.2f}", fontsize=10, color='black')
...
def SuspectedAdverseDrugEvents_count_for_eachdrug(prediction_combine):
    ...
    plt.xticks(ticks=xtick_positions, labels=prediction_combine['Drugs'],rotation=45, ha="right", fontsize=8, fontfamily='Arial')
...
def SuspectedAdverseDrugEvents_count(pre_benign, pre_toxic, list_of_adverse_reaction):
    ...
    ax.set_xticklabels(list_of_adverse_reaction, rotation=45, ha="right", fontsize=10, fontname='Arial')
