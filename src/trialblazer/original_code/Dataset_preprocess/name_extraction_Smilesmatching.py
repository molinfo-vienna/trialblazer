...
def ExtractingDrugsNamesAndSmilesMatching(dataset, Name_column):
    ...
    cirpy_result, GoToPubchempy = cirpy_SmilesMatching(FailToExtract, GoToSmilesMatching)
    pubchempy_result, GoTo_NextRound = pubchempy_SmilesMatching(GoToPubchempy, 'name_to_use')
    SmilesMatched = pd.concat([cirpy_result,pubchempy_result], ignore_index=True).reset_index()
    ...
    for row in LM_output:
        if row:
            ...
            if isinstance(first_tuple[0], dict):
                ...
                drugbank_id_append.append(first_tuple[0].get('drugbank_id', np.nan))
            else:
...
def cirpy_SmilesMatching(FailToExtract, GoToSmilesMatching):
    ...
    for df in [FailToExtract, GoToSmilesMatching]:
        for i, row in df[name].items():
            try:
                ...
                smiles_append.append(np.nan)
        name = 'name_to_use'
    GoToSmileMatching = pd.concat([FailToExtract,GoToSmilesMatching], ignore_index=True)
    ...
    if 'SMILES_ORI' in GoToPubchempy.columns:
        ...
        cirpy_result = pd.concat([cirpy_result, cirpy_result_2], ignore_index=True)
...
def Extract_drug_name(input_df, input_name):
    ...
    input_df = input_df[(input_df['replace_Name'].str.startswith(tuple(name_NeedToExcluded)) == False)]
