...
def remove_duplicate_splitted_files(inputfolder,outputfolder,prefix):
    ...
    print('number of entries after drop duplicated inchi: {}'.format(str(len(df))))
    ...
    df_noVariance[['preprocessedSmiles','id','conID','tautomerizedSmiles', 'inchi_noStereo']].to_csv(outputFile1,sep='\t',index=False)
...
def split_dataframe_by_groups_with_tautomerizedSmiles(df,outputFile,size=5000):
    ...
    for name,group in df:
        ...
        if len(frames) % size == 0:
            ...
            results_df[['preprocessedSmiles','id','conID','tautomerizedSmiles', 'inchi_noStereo']].to_csv(outputFile3,sep='\t',index=False)
            del results_df
            frames = []
    i += 1
    results_df = remove_smiles_fully_undefined_stereo(pd.concat(frames))
