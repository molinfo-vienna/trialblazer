...
from .neutralizationPatterns import neutralizationReactions
...
class MoleculePreprocessor:
    ...
    @classmethod
    def init_with_smiles(
        cls,
        listOfSmiles,
        replaceFilteredSmiles=True,
        removeSteroInfo=False,
    ):
            ...
            if removeSteroInfo:
                try:
                    mol = Chem.MolFromSmiles(
                        Chem.CanonSmiles(raw_smiles, useChiral=0),
                    )
    ...
    def salt_filter(self):
        ...
        for key in self.dictOfMolecules:
            ...
            sortSmileSplitList = sorted(
                smileSplitList,
                key=lambda x: x[1],
                reverse=True,
            )
            if (
                len(sortSmileSplitList) > 1
                and sortSmileSplitList[0][1] * 0.7 < sortSmileSplitList[1][1]
                and (
                    not Chem.MolToSmiles(sortSmileSplitList[0][0], True)
                    == Chem.MolToSmiles(sortSmileSplitList[1][0], True)
                )
            ):
    ...
    def get_rawsmiles_smiles_dict(self):
        ...
        print("this step is get_rawsmiles_dic")
        return dictOfSmiles
