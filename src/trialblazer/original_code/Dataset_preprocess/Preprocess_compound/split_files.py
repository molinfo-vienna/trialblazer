...
def split_large_files(file, filesize):
    ...
    smallfile = None
    with open(file, encoding="utf8") as bigfile:
...
if __name__ == "__main__":
    ...
    p.add_argument(
        ...
        "--inputfilepath",
        type=str,
        help="Specify input filepath with files to split.",
        default="/data/local/data_food/check_structure/Preprocess",
    )
    p.add_argument(
        "-n",
        "--splitnumber",
        type=int,
        help="Number of molecules in one file.",
        default=3000,
    )
    args = p.parse_args()
    ...
    for file in os.listdir(args.inputfilepath):
        if file.endswith(".smi"):
            split_large_files(
                os.path.join(args.inputfilepath, file),
                args.splitnumber,
            )
