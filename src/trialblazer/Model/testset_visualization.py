...
def plot_score_distribution_with_significance(data, p_values):
    ...
    for i, ((cat1, cat2), p) in enumerate(p_values.items()):
        x1, x2 = (
            list(data["Category"].unique()).index(cat1),
            list(data["Category"].unique()).index(cat2),
        )
        y = y_max + ((i + 1) * step)
...
def plot_correlation(predict_result_toxic, predict_result_benign):
    ...
    all_logP = (
        predict_result_toxic["logP"].tolist() + predict_result_benign["logP"].tolist()
    )
    all_scores = (
    ...
    plt.text(
        633, 2.2, f"Threshold = {prediction_threshold:.2f}", fontsize=10, color="black"
    )
    plt.text(140, 32, f"r (MW) = {correlation_mw:.2f}", fontsize=10, color="black")
    plt.text(
        140, 30.5, f"r (logP) = {correlation_logP:.2f}", fontsize=10, color="black"
    )
...
def SuspectedAdverseDrugEvents_count_for_eachdrug(prediction_combine):
    ...
    for i, (column, color) in enumerate(
        zip(prediction_combine.columns[3:], colors),
    ):
        plt.bar(
            x_indexes + i * bar_width,
            prediction_combine[column],
            width=bar_width,
            label=column,
            color=color,
        )
    ...
    plt.xticks(
        ...
        labels=prediction_combine["Drugs"],
        rotation=45,
        ha="right",
        fontsize=8,
    )
    ...
    plt.legend(
        title="Severity", loc="upper center", bbox_to_anchor=(0.5, -0.25), ncol=5
    )
    plt.tight_layout()
    plt.show()
...
def SuspectedAdverseDrugEvents_count(
    pre_benign,
    pre_toxic,
    list_of_adverse_reaction,
):
    (
        benign_not_serious_list_array,
        benign_serious_list_array,
    ) = process_adverse_reactions(pre_benign, list_of_adverse_reaction)
    toxic_not_serious_list_array, toxic_serious_list_array = process_adverse_reactions(
        pre_toxic, list_of_adverse_reaction
    )
    ...
    for i, (label, counts) in enumerate(category_counts_benign.items()):
        ax.bar(
            ...
            counts,
            width=width,
            label=f"Predicted benign: {label}",
            color=base_color_benign,
            alpha=alphas[i],
            edgecolor="black",
            linewidth=1,
            bottom=bottom_benign,
        )
    ...
    for i, (label, counts) in enumerate(category_counts_toxic.items()):
        ax.bar(
            ...
            counts,
            width=width,
            label=f"Predicted toxic: {label}",
            color=base_color_toxic,
            alpha=alphas[i],
            edgecolor="black",
            linewidth=1,
            bottom=bottom_toxic,
...
def process_adverse_reactions(prediction, list_of_adverse_reaction):
    ...
    for reaction in list_of_adverse_reaction:
        not_serious_list.append(
            prediction.loc[
                prediction["Seriousness"] == "Non Serious",
                reaction,
            ].tolist(),
        )
        serious_list.append(
            prediction.loc[
                prediction["Seriousness"] == "Serious",
                reaction,
            ].tolist(),
        )
